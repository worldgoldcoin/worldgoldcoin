...
uint256 hashGenesisBlock("0x0000046d6e7b458d1903fcd32233a659dc96d8d2c8302282b252774b1f721afb");
...
const string strMessageMagic = "WorldGoldCoin Signed Message:\n";
...
{
    // WorldGoldCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    // WorldGoldCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "WorldGoldCoin Genesis";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1474638247;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1315605;
	    ...
	    assert(block.hashMerkleRoot == uint256("0x20cf6333ff91c9c6b1e3185d7d8d533c904698e9577e169c316c4da885afd562"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xc7, 0xd4, 0xa3, 0xef };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("WorldGoldCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("WorldGoldCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static WorldGoldCoinMiner(CWallet *pwallet)
{
    printf("WorldGoldCoinMiner not supported!\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("WorldGoldCoin-miner");
    ...
    {
        printf("WorldGoldCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&WorldGoldCoinMiner, pwallet));
}
